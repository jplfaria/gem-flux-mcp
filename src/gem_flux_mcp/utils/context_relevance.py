...
class SpecificationRelevanceScorer:
    ...
    def _detect_component_spec(
        self, task_description: str, task_analysis: Dict[str, any] = None
    ) -> str | None:
        ...
        if task_analysis and "phase" in task_analysis:
            ...
            if phase in [8, 9, 10, 11, 12, 13, 14, 15]:
                ...
                if phase_specs and phase_specs[0].endswith("-agent.md"):
                    ...
                    logger.info(
                        f"✓ Component spec detected via phase: {phase_specs[0]} (Phase {phase} agent implementation)"
                    )
        ...
        if task_analysis and "components" in task_analysis:
            ...
            if components:
                ...
                if primary_component in self.component_spec_map:
                    ...
                    logger.info(
                        f"✓ Component spec detected via analysis: {detected_spec} (from component: {components[0]})"
                    )
        ...
        for component_key, spec_file in self.component_spec_map.items():
            ...
            if re.search(pattern, task_lower):
                logger.info(
                    f"✓ Component spec detected via pattern matching: {spec_file} (keyword: {component_key})"
                )
                return spec_file
        ...
        if agent_match:
            ...
            if agent_type in self.component_spec_map:
                ...
                logger.info(
                    f"✓ Component spec detected via agent pattern: {detected_spec} (agent: {agent_type})"
                )
                return detected_spec
    ...
    def apply_domain_weighting(
        self, base_score: float, task_keywords: Set[str], spec_path: str
    ) -> float:
        ...
        if any(kw in task_keywords for kw in self.domain_keywords["tools"]):
            if any(
                tool in spec_path for tool in ["build-media", "build-model", "gapfill", "run-fba"]
            ):
    ...
    def select_optimal_specs(
        self, task_description: str, max_specs: int = 5
    ...
    def detect_task_type(self, task_description: str) -> str:
        ...
        if any(
            ...
            for keyword in ["test", "testing", "unit test", "integration test"]
        ):
            return "testing"
        elif any(
            keyword in task_lower
            for keyword in ["build_media", "build_model", "gapfill", "run_fba", "tool"]
        ):
            return "tool_implementation"
        elif any(
            keyword in task_lower for keyword in ["database", "compound", "reaction", "lookup"]
        ):
    ...
    def select_optimal_specs_with_analysis(
        self, task_description: str, task_analysis: Dict[str, any], max_specs: int = 6
    ) -> ContextRecommendation:
        ...
        for spec_file in os.listdir(self.specs_dir):
            ...
            if spec_file.endswith(".md") and spec_file not in selected_specs:
                ...
                enhanced_score = self.enhance_score_with_analysis(
                    base_score, spec_file, task_analysis
                )
        ...
        components_str = (
            ", ".join(task_analysis["components"][:3]) if task_analysis["components"] else "general"
        )
    ...
    def enhance_score_with_analysis(
        self, base_score: float, spec_file: str, task_analysis: Dict[str, any]
    ) -> float:
        ...
        if domain == "tools" and "tool" in spec_file:
            ...
            enhanced_score += 0.2
        elif domain == "modeling" and any(
            model in spec_file for model in ["model", "template", "media"]
        ):
            enhanced_score += 0.2
    ...
    def validate_context_selection(
        self, task: str, selected_specs: List[str], current_phase: int
    ) -> Dict[str, any]:
    ...
    def get_phase_requirements(self, phase: int) -> List[Dict[str, any]]:
        ...
        phase_map = {
            1: [
                {
                    ...
                    "keywords": ["installation", "setup", "dependencies", "uv"],
                }
            ],
            2: [
                {
                    "spec_type": "database",
                    "keywords": ["database", "compounds", "reactions", "modelseed"],
                }
            ],
            ...
            5: [
                {
                    "spec_type": "build-model",
                    "keywords": ["model", "genome", "template", "rast", "fasta"],
                }
            ],
