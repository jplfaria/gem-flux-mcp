...
class TestBuildMediaRequest:
    ...
    def test_valid_with_custom_bounds(self):
        ...
        request = BuildMediaRequest(
            ...
            default_uptake=50.0,
            custom_bounds={"cpd00027": [-5.0, 100.0], "cpd00007": [-10.0, 100.0]},
        )
    ...
    def test_zero_default_uptake(self):
        ...
        with pytest.raises(ValidationError):
            BuildMediaRequest(compounds=["cpd00027"], default_uptake=0.0)
    ...
    def test_invalid_custom_bounds_order(self):
        ...
        errors = exc_info.value.errors()
        assert any(
            "lower" in str(e["ctx"]["error"]) and "upper" in str(e["ctx"]["error"]) for e in errors
        )
...
class TestBuildMediaResponse:
    ...
    def test_valid_response(self):
        ...
        response = BuildMediaResponse(
            ...
            compounds=[
                CompoundInfo(
                    id="cpd00027", name="D-Glucose", formula="C6H12O6", bounds=[-5.0, 100.0]
                )
            ],
            num_compounds=1,
            media_type="minimal",
    ...
    def test_rich_media_type(self):
        ...
        response = BuildMediaResponse(
            ...
            compounds=[
                CompoundInfo(
                    id=f"cpd{i:05d}", name=f"Compound {i}", formula="C1H1O1", bounds=[-100.0, 100.0]
                )
                for i in range(60)
            ],
            num_compounds=60,
...
class TestBuildModelRequest:
    ...
    def test_valid_with_protein_sequences(self):
        ...
        request = BuildModelRequest(
            protein_sequences={"prot_001": "MKLVINLV", "prot_002": "MKQHKAMI"},
    ...
    def test_empty_sequence(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            BuildModelRequest(protein_sequences={"prot_001": ""}, template="GramNegative")
    ...
    def test_invalid_template(self):
        ...
        with pytest.raises(ValidationError):
            BuildModelRequest(
                protein_sequences={"prot_001": "MKLVINLV"}, template="InvalidTemplate"
            )
    ...
    def test_valid_response(self):
        ...
        response = BuildModelResponse(
            ...
            model_properties=ModelProperties(
                is_draft=True, requires_gapfilling=True, estimated_growth_without_gapfilling=0.0
            ),
        )
        assert response.success is True
...
class TestGapfillModelRequest:
    ...
    def test_valid_minimal_request(self):
        ...
        request = GapfillModelRequest(model_id="model_001.draft", media_id="media_001")
    ...
    def test_negative_target_growth_rate(self):
        ...
        with pytest.raises(ValidationError):
            GapfillModelRequest(
                model_id="model_001.draft", media_id="media_001", target_growth_rate=-0.01
            )
    ...
    def test_zero_target_growth_rate(self):
        ...
        with pytest.raises(ValidationError):
            GapfillModelRequest(
                model_id="model_001.draft", media_id="media_001", target_growth_rate=0.0
            )
    ...
    def test_invalid_gapfill_mode(self):
        ...
        with pytest.raises(ValidationError):
            GapfillModelRequest(
                model_id="model_001.draft", media_id="media_001", gapfill_mode="invalid_mode"
            )
    ...
    def test_valid_successful_response(self):
        ...
        response = GapfillModelResponse(
            ...
            atp_correction=ATPCorrectionStats(
                performed=True, media_tested=54, media_passed=54, media_failed=0, reactions_added=0
            ),
            genomescale_gapfill=GenomescaleGapfillStats(
                performed=True, reactions_added=4, reversed_reactions=0, new_reactions=4
            ),
...
class TestRunFBARequest:
    ...
    def test_valid_minimal_request(self):
        """Test minimal valid request with defaults."""
        request = RunFBARequest(model_id="model_001.draft.gf", media_id="media_001")
    ...
    def test_negative_flux_threshold(self):
        ...
        with pytest.raises(ValidationError):
            RunFBARequest(model_id="model_001.draft.gf", media_id="media_001", flux_threshold=-1e-6)
...
class TestErrorResponse:
    ...
    def test_minimal_error_response(self):
        ...
        error = ErrorResponse(error_type="InternalError", message="An error occurred")
        assert error.success is False
...
class TestTypeIntegration:
    ...
    def test_build_media_to_run_fba_flow(self):
        ...
        fba_request = RunFBARequest(model_id="model_001.draft.gf", media_id=media_response.media_id)
    ...
    def test_build_model_to_gapfill_flow(self):
        ...
        model_response = BuildModelResponse(
            ...
            model_properties=ModelProperties(is_draft=True, requires_gapfilling=True),
        )

        # Use model_id in gapfill request
        gapfill_request = GapfillModelRequest(
            model_id=model_response.model_id, media_id="media_001"
        )
